# Términos de Servicio de Givlyn

**Última actualización:** 20 de noviembre de 2025

## 1. Aceptación de los Términos

Al acceder y usar Givlyn ("el Servicio"), operado por Wincova Corporation ("nosotros", "nuestro"), aceptas estar sujeto a estos Términos de Servicio ("Términos"). Si no estás de acuerdo con alguna parte de estos términos, no puedes usar nuestro Servicio.

## 2. Descripción del Servicio

Givlyn es una plataforma web que permite a los usuarios:
- Crear y gestionar listas de regalos
- Organizar grupos de personas
- Planificar eventos especiales
- Buscar y guardar ideas de regalos
- Compartir listas con otros usuarios

## 3. Elegibilidad

Debes tener al menos 16 años para usar este Servicio. Al registrarte, declaras que:
- Tienes la edad legal requerida
- Toda la información proporcionada es veraz y precisa
- Mantendrás actualizada tu información de cuenta

## 4. Registro de Cuenta

### 4.1 Creación de Cuenta
Para usar ciertas funciones, debes crear una cuenta proporcionando:
- Dirección de correo electrónico válida
- Nombre de usuario
- Contraseña segura

### 4.2 Seguridad de Cuenta
Eres responsable de:
- Mantener la confidencialidad de tu contraseña
- Todas las actividades que ocurran bajo tu cuenta
- Notificarnos inmediatamente sobre cualquier uso no autorizado

## 5. Uso Aceptable

### 5.1 Está Permitido:
- Usar el Servicio para fines personales y no comerciales
- Crear y compartir listas de regalos
- Colaborar con otros usuarios en grupos
- Guardar y organizar ideas de regalos

### 5.2 Está Prohibido:
- Usar el Servicio para actividades ilegales
- Publicar contenido ofensivo, difamatorio o discriminatorio
- Intentar acceder a cuentas de otros usuarios
- Realizar ingeniería inversa del software
- Automatizar el acceso (bots, scrapers) sin autorización
- Compartir contenido que viole derechos de propiedad intelectual
- Distribuir virus, malware o código malicioso
- Interferir con el funcionamiento del Servicio

## 6. Contenido del Usuario

### 6.1 Propiedad
Retienes la propiedad del contenido que publicas (listas, comentarios, descripciones).

### 6.2 Licencia
Al publicar contenido, nos otorgas una licencia mundial, no exclusiva, libre de regalías para:
- Almacenar y mostrar tu contenido
- Hacer copias de respaldo
- Proporcionar el Servicio según lo esperado

### 6.3 Responsabilidad
Eres responsable del contenido que publicas. No publicaremos ni eliminaremos contenido arbitrariamente, excepto cuando:
- Viole estos Términos
- Infrinja derechos de terceros
- Sea requerido por ley

## 7. Propiedad Intelectual

### 7.1 Nuestros Derechos
El Servicio, su diseño, código, marca y contenido son propiedad de Wincova Corporation y están protegidos por derechos de autor, marcas registradas y otras leyes. Givlyn es una marca de Wincova Corporation.

### 7.2 Marcas de Terceros
Las marcas de productos y servicios mencionados pertenecen a sus respectivos propietarios.

## 8. Privacidad

Tu privacidad es importante. Consulta nuestra [Política de Privacidad](./PRIVACY_POLICY.md) para entender cómo recopilamos, usamos y protegemos tu información.

## 9. Modificaciones al Servicio

Nos reservamos el derecho de:
- Modificar o descontinuar el Servicio (temporal o permanentemente)
- Cambiar características o funcionalidades
- Establecer límites de uso

Te notificaremos sobre cambios significativos con al menos 30 días de anticipación.

## 10. Terminación

### 10.1 Por tu Parte
Puedes cancelar tu cuenta en cualquier momento desde la configuración de tu perfil.

### 10.2 Por Nuestra Parte
Podemos suspender o terminar tu cuenta si:
- Violas estos Términos
- Tu uso pone en riesgo al Servicio o a otros usuarios
- Lo requiere la ley

### 10.3 Efectos de la Terminación
Al terminar tu cuenta:
- Perderás acceso al Servicio
- Tu contenido puede ser eliminado (sujeto a obligaciones legales de retención)
- Las secciones que sobreviven a la terminación siguen vigentes

## 11. Descargo de Responsabilidad

EL SERVICIO SE PROPORCIONA "TAL CUAL" Y "SEGÚN DISPONIBILIDAD". NO GARANTIZAMOS QUE:
- El Servicio esté libre de errores o interrupciones
- Los defectos serán corregidos
- El Servicio cumpla tus requisitos específicos

EN LA MÁXIMA MEDIDA PERMITIDA POR LEY, RECHAZAMOS TODAS LAS GARANTÍAS, EXPRESAS O IMPLÍCITAS.

## 12. Limitación de Responsabilidad

EN NINGÚN CASO SEREMOS RESPONSABLES POR:
- Daños indirectos, incidentales o consecuentes
- Pérdida de beneficios, datos o oportunidades de negocio
- Daños que excedan la cantidad que hayas pagado (si aplica) en los últimos 12 meses

Algunas jurisdicciones no permiten limitaciones de responsabilidad, por lo que estas limitaciones pueden no aplicarte.

## 13. Indemnización

Aceptas indemnizarnos contra reclamaciones, daños, pérdidas y gastos (incluidos honorarios legales) derivados de:
- Tu uso del Servicio
- Tu violación de estos Términos
- Tu violación de derechos de terceros

## 14. Enlaces a Terceros

El Servicio puede contener enlaces a sitios web de terceros. No somos responsables del contenido, políticas o prácticas de sitios de terceros.

## 15. Ley Aplicable y Jurisdicción

Estos Términos se rigen por las leyes de [País/Estado], sin considerar conflictos de principios legales.

Cualquier disputa será resuelta en los tribunales de [Ciudad, País].

## 16. Resolución de Disputas

### 16.1 Negociación Informal
Antes de iniciar acciones legales, contacta a: legal@giftapp.com para intentar resolver la disputa informalmente.

### 16.2 Arbitraje
Si la negociación falla, las disputas se resolverán mediante arbitraje vinculante, excepto cuando la ley local lo prohíba.

## 17. Cambios a los Términos

Podemos actualizar estos Términos ocasionalmente. Te notificaremos sobre cambios materiales:
- Mediante correo electrónico
- Con un aviso en el Servicio

El uso continuado después de los cambios constituye aceptación de los nuevos términos.

## 18. Divisibilidad

Si alguna disposición de estos Términos se considera inválida, las demás disposiciones permanecerán en pleno vigor.

## 19. Renuncia

Ninguna renuncia a cualquier término será considerada como una renuncia continua de ese término o de cualquier otro término.

## 20. Acuerdo Completo

Estos Términos, junto con la Política de Privacidad, constituyen el acuerdo completo entre tú y GiftApp respecto al uso del Servicio.

## 21. Contacto

Para preguntas sobre estos Términos:

**Razón Social**: Wincova Corporation  
**Email Legal**: legal@givlyn.com  
**Email de Soporte**: support@givlyn.com  
**Teléfono**: +1 615-728-9932

**Dirección Postal**:  
Wincova Corporation  
2615 Medical Center Parkway, Suite 1560  
Murfreesboro, TN 37129  
United States

## 22. Idioma

En caso de conflicto entre la versión en español y cualquier traducción, la versión en español prevalecerá.

---

**Versión**: 2.0  
**Fecha de entrada en vigor**: 20 de noviembre de 2025  
**Entidad Legal**: Wincova Corporation  
**Marca**: Givlyn (givlyn.com)

Al usar Givlyn, confirmas que has leído, comprendido y aceptado estos Términos de Servicio.
